...
db.serialize(() => {
  ...
  // Normalize city cache keys for lookup (lowercase trimmed)
  ...
  // Merge seed city file to improve coverage
  try {
    const seedPath = path.join(__dirname, 'mexico_cities_seed.json');
    if (fs.existsSync(seedPath)) {
      const seed = JSON.parse(fs.readFileSync(seedPath, 'utf8'));
      // seed is an array of {name, lat, lon, aliases}
  ...
  function findFuzzyMatch(city) {
    ...
    for (const k of Object.keys(cityCache)) {
      // if k contains key or key contains k
    ...
    // accept if relative distance <= 0.35
    if (best.key && best.score <= 0.35) {
      return { key: best.key, coords: cityCache[best.key], score: best.score };
    }
    return null;
  }
